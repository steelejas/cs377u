...
export var renderSurveysPreview = function (survey, root, displayState, previewQuestionIndex) {
    ...
    Preact.render(<Surveys key={'surveys-render-preview'} survey={survey} readOnly={true} initialDisplayState={displayState} previewQuestionIndex={previewQuestionIndex} style={{
            position: 'relative',
            right: 0,
...
export function Questions(_a) {
    ...
    var _f = __read(useState({}), 2), questionsResponses = _f[0], setQuestionsResponses = _f[1];
    var _g = useContext(SurveyContext), readOnly = _g.readOnly, previewQuestionIndex = _g.previewQuestionIndex;
    var _h = __read(useState(readOnly ? previewQuestionIndex : 0), 2), currentQuestion = _h[0], setCurrentQuestion = _h[1];
    ...
    var questionToDisplay = readOnly ? previewQuestionIndex : currentQuestion;
    var hasMultipleQuestions = survey.questions.length > 1;
    var isPreviewThankYouMessage = readOnly && questionToDisplay === survey.questions.length;
    return (<form 
    // TODO: BEMify classes
    className="survey-form" style={__assign(__assign({ color: textColor, borderColor: (_c = survey.appearance) === null || _c === void 0 ? void 0 : _c.borderColor }, styleOverrides), (isPreviewThankYouMessage ? { border: 'none', borderBottom: 'solid 1px rgb(201, 198, 198)' } : {}))}>
            {isPreviewThankYouMessage ? (<ConfirmationMessage confirmationHeader={((_d = survey.appearance) === null || _d === void 0 ? void 0 : _d.thankYouMessageHeader) || 'Thank you!'} confirmationDescription={((_e = survey.appearance) === null || _e === void 0 ? void 0 : _e.thankYouMessageDescription) || ''} appearance={survey.appearance || defaultSurveyAppearance} styleOverrides={__assign(__assign({}, style), { position: 'relative', right: '0px', borderRadius: '10px', border: 'solid 1px rgb(201, 198, 198)' })} onClose={function () { }}/>) : (<>
                    {surveyQuestions.map(function (question, idx) {
                if (hasMultipleQuestions) {
                    return (<>
                                    {questionToDisplay === idx && (<div className={"tab question-".concat(idx, " ").concat(question.type)}>
                ...
                return questionTypeMap(surveyQuestions[idx], idx, survey.appearance || defaultSurveyAppearance, function (res) { return onNextClick(res, idx); }, function () { return closeSurveyPopup(survey, posthog, readOnly); });
            })}
                </>)}
        </form>);
}
